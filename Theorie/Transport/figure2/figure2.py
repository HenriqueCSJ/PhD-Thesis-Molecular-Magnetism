...
#trace du p(\mu)
x = linspace(-1,1,1500)
ax1.plot(x,fer(x,0,1,0.001))
ax1.plot(x,fer(x,0,1,0.01))
ax1.plot(x,fer(x,0,1,0.05))
ax1.plot(x,fer(x,0,1,0.1))
...
N = range(-3,4)
for X in N :
	ax2.plot(VG,U(X,VG),'black',linewidth=1)

for X in N :
	temp = linspace(X-0.5,X+0.5,500)
	ax2.plot(temp,U(X,temp),'black')
